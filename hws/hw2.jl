...
# ╔═╡ d66d61c8-2705-4c32-a297-cd8afc96af54
md"""
...
# ╔═╡ a04fd222-e687-4853-906e-8c3b47993c73
md"""
...
function convolve(v::AbstractVector, k)
	...
	k_size = length(k)
	v_size = length(v)
	# Offset the indices vector
	l_range = k_size ÷ 2
	# Offset the indices of the kernel vector for convenience
	k_offset = OffsetArray(k, -l_range:l_range)
	# convoluted vector
	v_conv = []
	# Apply the kernel vector k to every element in v
	for vᵢ in 1:v_size
		vₙ = 0
		for m in -l_range:l_range
			vₙ += extend(v, vᵢ+m)*k_offset[m]
		end
		# Append the convoluted values
		push!(v_conv, vₙ)
	end
	return v_conv
end
