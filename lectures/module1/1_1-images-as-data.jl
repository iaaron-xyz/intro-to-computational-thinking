...
begin
	url_img_1 = "https://images.unsplash.com/photo-1659535973636-6cef468d093b?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80"
	url_img_2 = "https://images.unsplash.com/photo-1661347334008-da5448d2a6cf?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80"
end

# ╔═╡ 9180fbcc-601e-11eb-0c22-c920dc7ee9a9
md"""
**Step 2**: Now we use the aptly-named `download` function to download the image file to our own computer. If we have the images already on our local PC, step 1 can be skipped and load the image directly.

- `img_1` and `img_2` are from the web while `img_3` is from the local environment.
"""

# ╔═╡ 34ffc3d8-601e-11eb-161c-6f9a07c5fd78
begin
	img_1 = download(url_img_1) # download to a local file. The filename is returned
	img_2 = download(url_img_2)
end

# ╔═╡ eeb52594-ecf2-40c9-9cde-cc14c0c996a3
rects = load("D:\\Mis archivos\\Coding\\ocw-mit\\intro-to-computational-thinking\\spring2021\\module1\\lectures\\images\\original\\rects01.png")
...
img_ssd_beach = load(img_1)

# ╔═╡ 02777458-3723-4fe2-9df2-b1f7041a711b
img_pc_inside = load(img_2)

# ╔═╡ e86ed944-ee05-11ea-3e0f-d70fc73b789c
md"Images of a ssd and a motherboard directly from the internet!"

# ╔═╡ c99d2aa8-601e-11eb-3469-497a246db17c
md"""
We see that the Pluto notebook has recognised that we created an object representing an image, and automatically displayed the resulting image!
"""

# ╔═╡ 11dff4ce-6bca-11eb-1056-c1345c796ed4
md"""
- [x] Exercise : change the url.
- [x] Exercise: download a file that is already on your own computer.
"""
...
ssd_size = size(img_pc_inside)

# ╔═╡ 20f8219d-cf28-4229-83cb-f1677dced2ea
rects_size = size(rects)
...
rects_height = rects_size[1]

# ╔═╡ f08d02af-6e38-4ace-8b11-7af4930b64ea
rects_width = rects_size[2]
...
@bind row_i Slider(1:size(rects)[1], show_value=true)

# ╔═╡ 6511a498-7ac9-445b-9c15-ec02d09783fe
@bind col_i Slider(1:size(rects)[2], show_value=true)
...
Notes:
- `trunc(type, input)`: casts input into the type `type`. 
"""

# ╔═╡ f0796032-8105-4f6d-b5ee-3647b052f2f6
rects[550:650, 1:trunc(Int, rects_width/2)] # display the [vertical range, horisontal range]
...
Note:
- `collect(low, top)`: display a list of numbers between low and top, inclusive. Like `range` in python. 
"""
...
rects_center = rects[380:700, 800:1120]
...
@bind range_rows RangeSlider(1:size(rects_center)[1])

# ╔═╡ 85919db9-1444-4904-930f-ba572cff9460
@bind range_cols RangeSlider(1:size(rects_center)[2])

# ╔═╡ 2ac47b91-bbc3-49ae-9bf5-4def30ff46f4
nose = rects_center[range_rows, range_cols]
...
# ╔═╡ 296e9b23-0fdd-40c8-9989-aa5d6e9ff15a
RGB(0, 1, 0)

# ╔═╡ ce31a961-1cf5-4c3b-80c5-e6044d6cff73
RGB(0, 0, 1)

# ╔═╡ b416bf6f-5dbf-44dc-a1aa-1a207e099dbe
RGB(1, 1, 0.5)
...
let
	temp = copy(rects_center)
	temp[100, 250] = RGB(1.0, 0.0, 0.0)
	temp
end
...
let
	temp = copy(rects_center)
	temp[100, 150:250] .= RGB(1.0, 0.0, 0.0)
	temp
end
...
reduced_image = rects[1:10:end, 1:10:end]
...
> **Answer**: A kind of linear interpolation to avoid that hardness.
"""
...
save("reduced_rects.png", reduced_image)
...
# ╔═╡ 74d01829-9ef1-4831-9a28-206a3961d7f9
[RGB(0, x, x) for x in 0:0.1:1]
...
[rects_center  rects_center]

# ╔═╡ 8433b862-60ee-11eb-0cfc-add2b72997dc
[rects_center                   reverse(rects_center, dims=2)
 reverse(rects_center, dims=1)  rot180(rects_center)]
...
# ╔═╡ ba026dcb-e045-4e75-b71c-f55e7499d118
md"""
#### Solution
"""


# ╔═╡ e8714a2d-6334-4a24-9e1c-e40de4fb2724
@bind red_value Slider(0:0.1:1, show_value=true)

# ╔═╡ d52f6f87-63a0-45f8-b3d5-507f59a60ef1
@bind green_value Slider(0:0.1:1, show_value=true)

# ╔═╡ 3ceed46e-8a0e-46e7-84b4-1de2efb948a3
@bind blue_value Slider(0:0.1:1, show_value=true)

# ╔═╡ 5f3f3406-e362-4dbb-986f-24d93acd099d
